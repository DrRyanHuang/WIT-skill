...
- **Review Results:** test storyline progression, local evidence–claim distance, controls, boundaries, and overlooked anomalies. For method or system papers, also audit whether Figure 1 plus its caption conveys the central idea; whether a compact worked case shows how the method operates, why it succeeds, why existing methods fail on the same case, and what mechanism creates the difference; and whether benchmark gains are localized through difference-focused case/subset analysis rather than reported only as aggregate metrics.
...
- **Method-paper explanatory evidence:** Figure 1 should normally compress the global idea; benchmark statistics establish whether and by how much the method works; a compact worked case should explain how the method operates, why it succeeds, and why existing methods fail; and difference-focused case/subset analysis should reveal where the aggregate gain comes from. Prefer the simplest mechanism-revealing case and systematic differential patterns over cherry-picked examples.
